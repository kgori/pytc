...
from ..errors.errors import TreeBuildingError
...
class TreeCollection(TreeSoftware):
    ...
    def __init__(self, records, tmpdir, supplied_binary=''):
        ...
        super(TreeCollection, self).__init__(records[0], tmpdir, supplied_binary)
        
        for record in records:
            if not record.dv:
                print 'Calculating distances [{0}]...'.format(
                    record.name if record.name else 'noname?')
                record.calculate_distances(tmpdir)

        self.record = concatenate(records)
        self.record.name = 'tc_sequences'
